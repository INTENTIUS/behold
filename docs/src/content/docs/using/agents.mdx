...
4. **inspect** — a node's `sourceLoc.file` is the typed source that declared it. To change the estate, edit there: chant is the source of truth, not behold.
