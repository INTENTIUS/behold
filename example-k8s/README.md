# The k3d turnkey demo: `behold demo k8s`
...
cluster (idempotent, reusing one already up), then `behold serve --local`
points its live overlay at it. `--local`'s usual boot step (`chant emulator
up`) is a no-op here, since chant's k8s lexicon has no local-emulator capability
...
1. Open **http://localhost:4600**. The graph shows the Deployment, Service and
   PodDisruptionBudget in **blue** (declared, not yet deployed).
...
**Managed-fields drift.** Scale or label the Deployment out of band, and
