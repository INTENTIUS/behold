...
describe("fly pill (#74)", () => {
  it("appears for a fly-lexicon project, naming the targeted endpoint when set", async () => {
    config(["fly"]);
  ...
  it("says real Fly when the var is unset, and is absent without the lexicon", async () => {
    config(["fly"]);
...
describe("temporal pill (#74)", () => {
  it("blocked with the actionable message when no temporal.profiles is declared", async () => {
  ...
  it("on-demand when profiles are declared", async () => {
