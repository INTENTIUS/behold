...
const CHANT = installed("@intentius/chant", "0.44.2", "bin/chant");
...
describe("diagnose", () => {
  it("passes every line for a healthy project — installed chant + lexicon, an env, committed Ops", async () => {
    const dir = fixture({
      ...
      "package.json": JSON.stringify({ name: "healthy", dependencies: { "@intentius/chant": "^0.44.2" } }),
      "src/main.ts": "",
      "ops/deploy.op.ts": `export const op = { name: "prod-apply", kind: ApplyOp };`,
      ...CHANT,
      ...installed("@intentius/chant-lexicon-aws", "0.44.2"),
    });
    ...
    expect(by(report, "chant").detail).toContain("chant 0.44.2");
    expect(by(report, "lexicons").detail).toContain("aws 0.44.2");
  ...
  it("fails, naming npm install, when the project's own chant and lexicons aren't installed", async () => {
    const dir = fixture({
      ...
      "package.json": JSON.stringify({ name: "uninstalled", dependencies: { "@intentius/chant": "^0.44.2" } }),
    });
  ...
  it("warns rather than fails for a k8s project with no kube context — the declared graph still serves", async () => {
    const dir = fixture({
      ...
      ...CHANT,
      ...installed("@intentius/chant-lexicon-k8s", "0.44.2"),
    });
  ...
  it("diagnoses an estate root across its members, and points serve at them rather than the root", async () => {
    ...
    expect(by(report, "chant").detail).toBe("a: chant 0.44.2, b: chant 0.44.2 (behold's floor 0.38.0)");
