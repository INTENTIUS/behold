...
function appFor(dir: string, env?: string) {
  const broadcaster = new Broadcaster();
  return createApp({ projectDir: dir, port: 0, ...(env ? { env } : {}) }, broadcaster, new FrameBuffer(), new OpRunner({ projectDir: dir, broadcaster, onDone: () => {} }));
}
...
afterAll(() => {
  for (const d of [designated, broken, noWorkflow]) rmSync(d, { recursive: true, force: true });
});
