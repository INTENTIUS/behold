...
try {
  ...
  try {
    ...
    //
    // The committed report is a chant#1636 one (schema v1, with the boundary
    // edge lists), so this is the KNOWN path: the survivor is named. The
    // counts-only fallback — every chant that predates #1636 — is
    // web/carve-steps.test.js's, since it needs a report this repo no longer
    // ships.
    ...
    check("Pick names the survivor the cut would strand", cutText.includes("aws_lambda_function.api reads bucket"));
    check("…and what bridging it costs", cutText.includes("data source") && cutText.includes("needed immediately"));
    check("…with no counts-only disclaimer, because this report has the lists", !honestyText.includes("chant#1636"));
    if (!cutText.includes("aws_lambda_function.api")) console.error("  cut:", JSON.stringify(cutText), "honesty:", JSON.stringify(honestyText));
